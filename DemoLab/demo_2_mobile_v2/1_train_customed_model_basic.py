...
def train_model(
    ...
    print_colored_box("最佳验证准确率: {:4f}".format(best_acc), attrs=['bold'], text_color='green', box_color='yellow')
...
def main():
    ...
    # 获取数据集
    train_dataset, val_dataset, _ = get_dataset(train_transform=train_transform)
    
    # 设置批次大小
    train_batch_size = args.batch_size if args.batch_size else quant_config.train_batch_size
    val_batch_size = args.batch_size if args.batch_size else quant_config.val_batch_size
    ...
    # 自动查找最新的检查点文件
    if args.auto_resume and os.path.isdir(quant_config.export_work_dir):
        ...
        if latest_checkpoint:
            ...
            logger.info(f"在目录 {quant_config.export_work_dir} 中未找到检查点文件，将从头开始训练。")
    ...
    logger.info(f"最终模型已保存到 {quant_config.export_work_dir}/{formatted_timestamp}_mobilev2_model.pth")
