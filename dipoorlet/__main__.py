...
parser.add_argument("-N", "--data_num", default=200, help="num of calibration pics", type=int)
...
parser.add_argument("--output_debug_dir", type=str, default="/mnt/share_disk/bruce_trie/Quantizer-Tools/outputs/dipoorlet_debug_onnx_models")
...
# dist.barrier()
# if dist.get_rank() == 0:
reduce_clip_val(1, args) # 对于多GPU的情况，需要将tensor calibration的结果进行reduce
# dist.barrier()
...
# dist.barrier()
# if dist.get_rank() == 0:
layer_cosine_dict, model_cosine_dict = reduce_profiling_res(1, args)
