...
def learning_round_mask(in_tensor, fp_out_tensor, ada_layer, reg, batch_size, max_epoch, debug):
    ...
    for epoch in range(1):
        for idx in range(np.ceil(len(in_tensor) / batch_size).astype(int)):
            ...
            if debug:
                print("the st is ", st)     # 0
                print("the ed is ", ed)     # 64
                # torch.Size([1, 3, 720, 1920])
                print("Input shape: ", input.shape) 
                
                # torch.Size([1, 32, 368, 960])
                print("FP Output shape:", fp_output.shape)
                # torch.Size([1, 32, 360, 960])
                print("the output is ", output.shape)
                
                # torch.Size([1, 32, 368, 960])
                print("the fp_output is ", fp_output.shape)
